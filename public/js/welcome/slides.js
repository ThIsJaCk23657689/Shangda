...
/***/ "./resources/js/welcome/slides.js":
/*!****************************************!*\
  !*** ./resources/js/welcome/slides.js ***!
  \****************************************/
...
$(document).ready(function () {
  ...
  var $body = $('body'); //add window a trigger

  setTimeout(function () {
    $(window).trigger('ready');
  }, 1); //Redraw

  $body.hide().show(0); //Detect mode
  ...
  $html.addClass('page-ready'); //Set speed
  ...
  window.stage = 1;
  window.stages = $('.slide').length; //Horizonal Mode
  ...
  function updateHash() {
    ...
    if (hashLink) {
      ...
      if ($('.slide[data-name="' + hashLink + '"]').length > 0) {
        ...
        var requestedElement = $('.slide[data-name="' + hashLink + '"]'); //scroll to a desired slide
  ...
  updateHash(); //Listen history changes
  ...
  if (window.preload) {
    ...
    var updateProgressBar = function updateProgressBar() {
      //loading
      var progress = window.loadingProgress / window.images; // animate
    ...
    window.progressBar = $('.progress-bar'); //preload images (sort of)
  ...
  function showSlide(requested) {
    ...
    var newSlide = $('.slide').eq(requested - 1),
        ...
        currenSlideIndex = currenSlide.index('.slide') + 1; //cleanup

    hideDropdown();
    unzoomImage();
    hideSidebar();
    window.allowSlide = 1; //reset

    $body.removeClass('sidebarShown lastSlide firstSlide hidePanel-top hidePanel-bottom'); //It it first or last stage?
    ...
    if (currenSlideIndex !== requested && window.setStageClasses != 0) {
      ...
      newSlide.removeClass('before after').addClass('selected active'); //set order

      newSlide.prevAll('.slide').addClass('before').removeClass('after');
      newSlide.nextAll('.slide').addClass('after').removeClass('before'); //set a trigger
    ...
    if (window.loaded) {
      ...
      }, window.slideSpeed - window.effectOffset); //clear element animation on done
  ...
  $('html,body').on('DOMMouseScroll mousewheel scroll touchmove', function (event) {
    var $currentSection = $('.slide.selected .content'),
        ...
        minScrollToSlide = window.isFirefox && window.isWindows ? 200 : window.minScrollToSlide; //skip empty events

    if (!scrollsize) return; //scroll mode
    ...
    if (!window.isScroll && !(window.isMobile && window.isSimplifiedMobile)) {
      ...
      if (currentSectionHeight > window.windowHeight) {
        ...
        if (!window.sidebarShown && !window.popupShown && !window.blockScroll) {
          if (window.smoothScroll) {
            //lock default scroll
            event.preventDefault(); //smooth scroll
      ...
      if (Math.abs(window.collectScrolls) >= minScrollToSlide && window.allowSlide && !window.sidebarShown && !window.popupShown && !window.disableOnScroll) {
        window.collectScrolls = 0; //should we even..
  ...
  function updateScroll() {
    ...
    $('.slide').each(function (index, element) {
      var $element = $(element),
          ...
          allowToSelect = true; //checking first slide
      ...
      if (window.setStageClasses != 0) {
        if (halfOnScreen && allowToSelect) {
          ...
          if (window.stage !== elementIndex + 1 || !window.firstTimeTrigger) {
            window.stage = elementIndex + 1; //set a trigger
            ...
            $body.removeClassByPrefix("stage-").addClass('stage-' + window.stage); //white slide?
  ...
  $('.mobile .slides:not(.scroll):not(.simplifiedMobile), .slides.desktopSwipe').swipe({
    swipeStatus: function swipeStatus(event, phase, direction, distance) {
      ...
      window.allowSwipeDown = 1; //set height for current slide
  ...
  if ($('.panel.hideOnScroll').length > 0) {
    ...
    if (window.isScroll || window.isSimplifiedMobile) {
      var lastScrollTop,
          ...
          panelToHide = $('.panel.hideOnScroll'); //hide if height larger than screen size
      ...
      $(window).on('mousewheel', function (event) {
        ...
        lastScrollTop = scrollTop; //show on top and bottom
  ...
  */
  ...
  window.showPopup = function (id) {
    ...
    if (element.length > 0) {
      hideSidebar();
      $(element).addClass('visible'); //set a trigger
      ...
      $(element).find('.content').scrollTop(0);
      window.allowSlide = 0; //Add Popup ID in the stack

      if (!window.popupShown) window.popupShown = [];
      window.popupShown.push(popupID); //Autoplay Iframe
  ...
  function hidePopup(popupID) {
    ...
    if ($.isArray(window.popupShown)) {
      var popupToHide = popupID ? popupID : window.popupShown.slice(-1)[0],
          ...
          iframe = $element.find('iframe'),
          video = $element.find('video'); //stop iframe autoplay
      ...
      setTimeout(function () {
        ...
        $html.removeClass('popup_' + popupToHide); //remove last shown id
  ...
  if ($(sliderEl).length > 0) {
    $(sliderEl).each(function (index, element) {
      ...
      var $el = $(element),
          ...
          nextIndex = $el.find('.selected').index(); //set status
      ...
      if ($el.hasClass('autoplay')) {
        ...
        var autoplay = setInterval(function () {
          $el.trigger('next');
        }, interval); //stop interval on user interaction
      ...
      if ($el.hasClass('clickable') || $el.hasClass('autoplay')) {
        $el.on('click next', function (event) {
          var $el = $(this),
              ...
              clickTarget = event.target; //break

          if ($(clickTarget).data('slider-event') == "cancel") return; //uselect old
          ...
          $nextElement.removeClass('hide').addClass('selected'); //set status
  ...
  if ($controller.length > 0) {
    ...
    $controller.on('click', controllerSelector, function () {
      ...
      if (!$controllerElement.hasClass('selected')) {
        ...
        $slider.children('li').eq(nextIndex).removeClass('hide').addClass('selected'); //set status
  ...
  $('[data-slider-action]').click(function () {
    if ($(this).data('slider-id')) {
      var $this = $(this),
          ...
          isAnimated = $slider.hasClass('animated'); //detect direction
      ...
      $desiredElement.removeClass('hide').addClass('selected'); //set status
  ...
  */
  ...
  window.dropdownShown = false; //click
  ...
  function showDropdown($this, $isHover) {
    $isHover = typeof $isHover !== 'undefined' ? $isHover : false; //show
    ...
    var offset = $this.offset(),
        ...
        elementPosition = elementPosition.split(' '); //hide
    ...
    render: function render(api) {
      ...
      $(api.element).on('click touchstart', '.mail', function () {
        var subject = $(this).data('subject') ? $(this).data('subject') : "",
            ...
            url = $('.dropdown').data('url') ? $('.dropdown').data('url') : window.location.href; //open email
...
window.isiPad = navigator.userAgent.match(/iPad/i) !== null; //Detect OS

window.isWindows = navigator.platform.toUpperCase().indexOf('WIN') !== -1;
window.isOSX = navigator.platform.toUpperCase().indexOf('MAC') !== -1;
window.isLinux = navigator.platform.toUpperCase().indexOf('LINUX') !== -1; //Prepare for CSS Fixes
...
/***/ 2:
/*!**********************************************!*\
  !*** multi ./resources/js/welcome/slides.js ***!
  \**********************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\AppServ\www\Shangda\resources\js\welcome\slides.js */"./resources/js/welcome/slides.js");
